...
//-----------------------------------------------------------------------------
//  This block code will 
//      respond to the GET request to DELETE a particular Crime
//-----------------------------------------------------------------------------

/* GET Display the List of Crimes page. */
router.get('/delete/:id', (req, res, next) => {
    let id = req.params.id | 0;
    deleteStatement(res, id)
});

//  execute the query
function deleteStatement(res, id) {
    //  create a query request
    let sqlDelete = `delete from Crime.dbo.NewYork where ComplaintNum = ${id}`;
    request = new Request(sqlDelete, (err, rowCount, rows) => {
        if (err) {
            console.log(err);
        } else {
            res.render('index', {
                title: `Crime# ${request.crime_id} has been deleted`,
...
router.post('/create', (req, res, next) => {
    ...
    let crime = new Crime(req.body.ComplaintNum, req.body.Borough, req.body.FromDate, FromTime, req.body.OffenseDesc, req.body.DescriptionCd, req.body.PremiseType);
